    ...
    if (showReport && result) {
      const generatePDF = async () => {
        try {
          ...
          const file = new File([blob], 'Momentail_VIP_Health_Report.pdf', { type: 'application/pdf' });
          setPdfFile(file);
        } catch (e) {
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          ...
          <motion.div key="result" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
            ...
            {/* Medical Disclaimer (Main View) */}
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }} className="mt-6 mb-4 p-4 rounded-xl flex gap-3 items-start" style={{ background: '#FDF6EE', border: '1px solid #F4D3B3' }}>
              <AlertTriangle size={16} style={{ color: '#A27B5C', flexShrink: 0, marginTop: 2 }} />
              <div className="text-[11px] leading-relaxed" style={{ color: '#7E6B5A' }}>
                <p className="font-bold mb-1">{KO ? '의학적 면책 조항 (Medical Disclaimer)' : 'Medical Disclaimer'}</p>
                <p className="mb-1">
                  {KO 
                ...
                <div id="vip-report-content" className="flex flex-col bg-white">
                  ...
                  <div className="bg-white border-b border-gray-100 p-6 shrink-0">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <p className="text-[11px] text-gray-400 tracking-widest uppercase">Momentail DogEngine v2.4 System Record</p>
                      </div>
                  ...
                  <div className="p-6">
                    ...
                    <div className="mt-6 p-4 rounded-xl bg-gray-50 border border-gray-100 text-[10px] text-gray-400 leading-relaxed">
                      <p>
                        {KO 
                          ? "* 본 리포트는 Momentail DogEngine AI가 사용자의 기록을 바탕으로 생성한 건강 참고 자료이며, 수의사의 진단을 대신할 수 없습니다. 참고 문헌: 일반적인 반려동물 관리 지침 (AVMA, AKC 등). 이상 징후가 의심될 경우 동물병원을 방문하세요."
                          : "* This report is a health reference generated by Momentail DogEngine AI. It is not a substitute for professional veterinary diagnosis. References: General pet care guidelines (e.g., AVMA, AKC). Please visit a veterinary clinic if you suspect health issues."}
                      </p>
                    </div>
                  </div>
                </div>
